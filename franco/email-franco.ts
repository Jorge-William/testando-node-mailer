import EmailParams from "./franco.interface"


class NotificacaoDeEmail implements EmailParams {
  nome: string;
  cursos: string[]

  constructor( nome: string, cursos: string[] ) {
    this.nome = nome;
    this.cursos = cursos;
  }
                  ...
                  <tr>
                    <td>
                      <p style="color: #565656; padding: 0px 32px;">
                        ...
                        <spans style="font-weight: 700;"> ${this.cursos.map( ( curso ) => { `${curso}, ` } )}</spans> expirou.
...
export default NotificacaoDeEmail